...
function page(id){$$('.page,nav button').forEach(element=>element.classList.remove("active"));$$('nav button').forEach(button=>button.removeAttribute("aria-current"));$("#"+id).classList.add("active");const button=$(`nav button[data-page="${id}"]`);button?.classList.add("active");button?.setAttribute("aria-current","page");$("#main-content").focus();if(id==="companies"||id==="spontaneous")loadEstablishments();if(id==="spontaneous")loadSpontaneousResumes();if(id==="offers")loadOffers();if(id==="trash")loadTrash();if(id==="resumes"){loadResumes();loadBackups()}if(id==="statistics")loadStatistics()}
