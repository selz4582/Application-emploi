...
class BrowserJourneyTests(unittest.TestCase):
    ...
    def test_navigation_has_no_design_mockups(self):
        ...
        self.assertEqual(self.page.get_by_role("button",name="Télécharger le courriel .eml").count(),1)
    ...
    def test_original_resume_can_be_downloaded_from_its_card(self):
        documents=app.DATA/"documents"; documents.mkdir()
        content=b"PK\x03\x04cv-browser"; path=documents/"stored.docx"; path.write_bytes(content)
        app.store.execute("INSERT INTO resumes(filename,path,created_at,preferred) VALUES(?,?,?,1)",("CV candidature.docx",str(path),"2026-09-24"))
        self.page.get_by_role("button",name="Mes CV").click()
        card=self.page.locator(".resume-card",has_text="CV candidature.docx")
        with self.page.expect_download() as download_info: card.get_by_role("link",name="Télécharger").click()
        download=download_info.value
        self.assertEqual(download.suggested_filename,"CV candidature.docx")
        self.assertEqual(Path(download.path()).read_bytes(),content)
