...
class HttpSmokeTests(unittest.TestCase):
    ...
    def test_resume_can_be_downloaded_without_exposing_its_storage_name(self):
        ...
        resume_id=app.store.execute("INSERT INTO resumes(filename,path,created_at) VALUES(?,?,?)",("CV été.docx",str(stored),"2026-09-24"))
        with urllib.request.urlopen(f"{self.base}/api/resumes/{resume_id}/download") as response:
            downloaded=response.read(); headers=response.headers
        self.assertEqual(downloaded,content); self.assertEqual(headers.get_content_type(),"application/vnd.openxmlformats-officedocument.wordprocessingml.document")
        self.assertIn("CV%20%C3%A9t%C3%A9.docx",headers["Content-Disposition"]); self.assertNotIn("storage-name",headers["Content-Disposition"])
        with self.assertRaises(urllib.error.HTTPError) as caught: self.get("/api/resumes/999/download")
        self.assertEqual(caught.exception.code,404); caught.exception.close()

    def test_application_email_can_be_downloaded_but_is_never_sent(self):
        application=app.store.execute("INSERT INTO applications(position,email_to,email_subject,email_body,created_at) VALUES(?,?,?,?,?)",("Agent","rh@example.test","Candidature Agent","Bonjour", "2026-09-24"))
        with urllib.request.urlopen(f"{self.base}/api/applications/{application}/email") as response:
            content=response.read(); headers=response.headers
        self.assertEqual(headers.get_content_type(),"message/rfc822"); self.assertIn(f"candidature-{application}.eml",headers["Content-Disposition"])
        self.assertIn(b"Subject: Candidature Agent",content); self.assertIn(b"rh@example.test",content)
        self.assertIsNone(app.store.rows("SELECT sent_at FROM applications WHERE id=?",(application,))[0]["sent_at"])
